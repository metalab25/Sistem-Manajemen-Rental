...
@section('content')
        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header pb-sm-2">
                    <div class="row">
                        <div class="col-md-8">
                            <form method="GET" action="{{ route('cashouts.index') }}">
                                <div class="row">
                                    <div class="col-md-3">
                        ...
                        <div class="col-md-4">
                            <div class="btn-group sm-full-fw float-end mb-0" role="group" aria-label="Small button group">
                                <a href="{{ route('cashouts.print', request()->query()) }}" target="_blank"
                                    class="btn btn-dark">Cetak Data</a>
                                <a href="" class="btn btn-success">Export Data</a>
                                <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                    data-bs-target="#modalAddCashOut">Tambah Data</button>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table table-bordered table-striped mb-0">
                            ...
                            <tbody>
                                @foreach ($cashOuts as $cashout)
                                    <tr class="{{ $cashout->status == 0 ? 'table-danger' : '' }}">
                                        <td class="text-center align-middle">
                                            {{ ($cashOuts->currentPage() - 1) * $cashOuts->perPage() + $loop->iteration }}
                                        </td>
                                        <td class="align-middle">
                                            {{ tanggal_indonesia($cashout->transaction_at, false) }}
                                        </td>
                                        <td class="align-middle">
                                            @if ($cashout->type == 'Penyewaan')
                                                {{ $cashout->item . ' ' . tanggal_indonesia($cashout->rent_date, false) }}
                                            @else
                                                {{ $cashout->item }}
                                            @endif
                                            ...
                                            <td class="text-center align-middle">
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <ul class="dropdown-menu">
                                                        ...
                                                        <li>
                                                            <a href="{{ route('cashouts.edit', $cashout->id) }}"
                                                                class="dropdown-item py-2 align-middle">
                                                                Ubah Transaksi
                                                            </a>
                                                        </li>
                                                        ...
                                                        <li>
                                                            <a href="{{ route('cashouts.destroy', $cashout->id) }}"
                                                                class="dropdown-item py-2 align-middle"
    ...
    @include('dashboard.pembukuan.keluar.modal-add')
@endsection
